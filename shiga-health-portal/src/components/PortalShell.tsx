...
import { Landmark, BarChart3, PlayCircle, MessageCircle, X, Clock, HelpCircle, ChevronLeft, ChevronRight } from "lucide-react";
...
const VIDEO_BASE: Record<string, string> = {
  ...
  v3: "https://www.youtube.com/embed/dummy_id_3",
};

export function PortalShell() {
  // 状態管理
  const [current, setCurrent] = useState({ id: "t0", url: "", label: "1 はじめに" });
  const [pageKey, setPageKey] = useState(0);
  const [isChatOpen, setIsChatOpen] = useState(false);
  const [messages] = useState([{ role: "ai", content: "操作ガイドです。項目を選択してください。" }]);

  // 現在の表示内容の判定
  const activeViz = TABLEAU_VIZZES.find(v => v.id === current.id);
  const isVideoOrExtMode = ["v1", "v2", "v3", "ext_viz"].includes(current.id);

  // ガイドメニューからのアクション処理
  const handleAction = (item: any) => {
    if (item.type === "external_viz") {
      setCurrent({ id: "ext_viz", url: item.url, label: item.label });
      return;
    }
      ...
      <aside className="w-80 shrink-0 flex flex-col text-white shadow-xl z-20" style={{ backgroundColor: OMI_NAVY }}>
        ...
        <nav className="p-3 space-y-1 overflow-y-auto flex-1">
          ...
          {TABLEAU_VIZZES.map(v => (
            <button 
              key={v.id} 
              onClick={() => { setCurrent({ id: v.id, url: "", label: v.label }); setPageKey(0); }} 
              className={`flex w-full items-center gap-3 p-3 rounded-xl text-sm transition-all ${current.id === v.id ? "bg-sky-500/20 ring-1 ring-sky-400" : "text-white/60 hover:bg-white/5"}`}
            >
          ...
          {Object.keys(VIDEO_BASE).map((vid, idx) => (
            <button 
              key={vid} 
              onClick={() => { setCurrent({ id: vid, url: `${VIDEO_BASE[vid]}?start=0`, label: `動画${idx + 1}` }); setPageKey(0); }}
              className={`flex w-full items-center gap-3 p-3 rounded-xl text-sm ${current.id === vid ? "bg-white/10 text-white" : "text-white/40 hover:bg-white/5"}`}
            >
              <PlayCircle className="h-5 w-5" />動画{idx + 1}
            </button>
          ))}
        </nav>
      </aside>
