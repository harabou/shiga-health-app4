...
export function PortalShell() {
  const [current, setCurrent] = useState<CurrentView>({ mode: "tableau", id: "t0", label: "1　はじめに" });
    ...
    <div className="flex h-screen w-full bg-slate-50 font-sans overflow-hidden text-slate-900">
      <aside className="w-80 shrink-0 flex flex-col text-white shadow-xl z-20" style={{ backgroundColor: OMI_NAVY }}>
        <div className="p-6 border-b border-white/10 flex items-center gap-3">
          ...
          <h1 className="font-bold text-lg leading-tight">健康寿命分析ポータル</h1>
        </div>
        <nav className="p-3 space-y-1 overflow-y-auto flex-1">
          {TABLEAU_VIZZES.map((v) => (
            <button key={v.id} onClick={() => handleSelectViz(v.id, v.label)}
              className={`flex w-full items-center gap-3 p-3 rounded-xl text-sm transition-all ${current.mode === "tableau" && current.id === v.id ? "bg-sky-500/20 ring-1 ring-sky-400 text-white" : "text-white/60 hover:bg-white/5"}`}>
          ...
          {Object.keys(VIDEO_BASE).map((vid, idx) => (
            <button key={vid} onClick={() => handleSelectVideo(vid, idx)}
              className={`flex w-full items-center gap-3 p-3 rounded-xl text-sm ${current.mode === "video" && current.id === vid ? "bg-white/10 text-white" : "text-white/40 hover:bg-white/5"}`}>
              <PlayCircle className="h-5 w-5 shrink-0" />動画{idx + 1}
            </button>
          ))}

          <div className="pt-6 pb-2 px-3 text-[10px] text-white/30 font-bold uppercase tracking-widest">解析ツール</div>
          <button onClick={() => setCurrent({ mode: "simulator", label: "健康寿命シミュレーター" })}
            className={`flex w-full items-center gap-3 p-3 rounded-xl text-sm transition-all ${current.mode === "simulator" ? "bg-emerald-500/20 ring-1 ring-emerald-400 text-white" : "text-white/40 hover:bg-white/5"}`}>
        ...
        <div className="flex-1 relative bg-slate-100 overflow-hidden flex flex-col">
          {current.mode === "tableau" && activeViz && (
            <>
              <div className="flex-1 min-h-0">
                <TableauEmbed key={`${activeViz.id}-${pageKey}`} embedSrc={toTableauEmbedUrl(activeViz.viewPaths[pageKey])} title={current.label} pageKey={pageKey} />
              </div>
              {activeViz.viewPaths.length > 1 && (
                <div className="h-14 bg-white border-t flex items-center justify-between px-6 shrink-0 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
                  <button disabled={pageKey === 0} onClick={() => setPageKey((p) => p - 1)} className="flex items-center gap-2 text-sm font-medium hover:text-blue-600 disabled:opacity-20 transition-colors">
                    <ChevronLeft className="w-4 h-4" /> 前のデータ
                  </button>
                  <span className="text-xs font-bold text-slate-400 bg-slate-50 px-3 py-1 rounded-full border">PAGE {pageKey + 1} / {activeViz.viewPaths.length}</span>
                  <button disabled={pageKey >= activeViz.viewPaths.length - 1} onClick={() => setPageKey((p) => p + 1)} className="flex items-center gap-2 text-sm font-medium text-blue-600 hover:text-blue-700 disabled:opacity-20 transition-colors">
          ...
          {(current.mode === "video" || current.mode === "ext_viz") && (
            <div className="absolute inset-0 bg-white p-4">
              <iframe key={current.url} src={current.url} className="w-full h-full rounded-xl shadow-lg border border-slate-200" allow="autoplay; fullscreen" />
            </div>
          )}
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        {isChatOpen && (
          <div className="w-80 h-[500px] bg-white rounded-3xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden animate-in slide-in-from-bottom-2">
            <div className="p-4 bg-slate-900 text-white flex justify-between items-center shrink-0">
              ...
              <button onClick={() => setIsChatOpen(false)} className="hover:bg-white/10 p-1 rounded-full transition-colors"><X className="w-4 h-4" /></button>
            </div>
            <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50">
              <div className="space-y-2">
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider text-center border-b pb-2">動画シーン選択</p>
                {GUIDE_ITEMS.map((item, idx) => (
                  <button key={idx} onClick={() => handleGuideAction(item)} className="w-full text-left p-3 text-[11px] bg-white border border-slate-100 rounded-xl hover:border-blue-500 hover:text-blue-600 transition-all flex items-center gap-2 shadow-sm group">
        ...
        <button onClick={() => setIsChatOpen(!isChatOpen)} className="p-4 rounded-full bg-blue-600 text-white shadow-xl hover:bg-blue-700 hover:scale-110 active:scale-95 transition-all ring-4 ring-blue-600/10">
